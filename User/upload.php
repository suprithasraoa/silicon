...
//    var ddlshowval="10";
    ...
    document.getElementById('sTableRow').innerHTML="<thead><tr><th>#</th><th>Client</th><th>File </th><th>Year</th><th>Date</th><th>Description</th><th>Action</th></tr></thead><tbody>";
    var slno=0;
  var ahref="";
...
// echo (result);
  slno++;
    ...
    $("#sTableRow").append("<tr><td>"+slno+"</td><td style='font-size: 13px;'>"+field.CName+"</td><td style='font-size: 11px;'><a  target='_blank' href='../Client/Photos/"+field.FilePath+"''><i class='fa fa-fw fa-download'></i></a></td><td style='font-size: 13px;'>"+field.Year+"</td><td style='font-size: 13px;'>"+field.Date+"</td><td style='font-size: 13px;'>"+field.Description+"</td><td>"+ahref);
...
<script>
 ...
 var userid = $("#userid").val(); 
  var url = "sub/bind_list.php?cid="+pkval+"&userid="+userid+"&tbl=UploadScannedCopies";
  ...
  //alert(result);
  $.each(result, function(i, field) {
  //$("input[id=txtuploadfile]").val(field.FilePath); 
...
<section class="content">
         ...
         <?php $userid =$_SESSION["UserlogId"]; $query ="SELECT ''  AS Id,'--select--' AS Name UNION ALL SELECT Id,Name FROM Client where Active=1 and UserId='$userid' ";
