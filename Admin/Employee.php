...
if	(txtClientId==null)	{
    ...
    document.getElementById('sTableRow').innerHTML="<thead><tr><th>#</th><th>User</th><th>Client</th><th>Name</th><th>Designation</th><th>Action</th></tr></thead><tbody>";
    var slno=0;
	var ahref="";
...
//alert(url);
  ...
  // alert(field.Name);
    ...
    $("#sTableRow").append("<tr><td>"+slno+"</td><td style='font-size: 11px;'>"+field.UName+"</td><td style='font-size: 11px;'>"+field.CName+"</td><td style='font-size: 11px;'>"+field.EName+"</td><td style='font-size: 13px;'>"+field.DesignationId+"</td><td>"+ahref);
...
//alert(url);
  ...
  $("#txtClientId").val(clid);
	...
	$("#gtxtClientId").attr('class', 'form-group');
		 ...
		 var url = "sub/get_Counter.php?ctr=EmployeeCtr&field=" +txtClientId;
//alert(url);
    $.getJSON(url, function(result) {
    console.log(result);
    $.each(result, function(i, field) {
    var EmployeeCtr= field.EmployeeCtr;
	$("#txtSerial").val(EmployeeCtr);
   ...
   var txtmobile = $("#txtmobile").val(); 
	 ...
	 // alert(output);
	  var esu = document.getElementById("txtUserId");
               var strUsers = esu.options[esu.selectedIndex].value;
	  var es = document.getElementById("txtClientId");
               var strClient = es.options[es.selectedIndex].value;
...
//alert(dataString);
               if ($.trim(txtname).length > 0 && $.trim(txtAddress1).length > 0 && $.trim(txtCity).length > 0 && $.trim(reservation).length > 0 && $.trim(txtPanRef).length > 0 && $.trim(strClient) > 0 && $.trim(strUsers) > 0 && $.trim(strDesig) > 0 && $.trim(strState) > 0){
            $.ajax({
                    type: "GET",
...
if	(txtClientId==null)	{
  ...
  var url = "sub/bind_list.php?cid="+pkval+"&CltId="+txtClientId+"&userid="+userid+"&tbl=Employee";
//alert(url);
  $.getJSON(url, function(result) {
...
?>
		 ...
		 <select  id= "txtClientId" class="form-control" onchange="bindCounter();">
                    ...
                    <div class="form-group">
                      ...
                      <input type="Number" class="form-control" id="txtSerial" disabled>
                    </div>
...
?>
				   ...
				   <div class="col-sm-12">
                    <div class="form-group" id="gtxtDesig">
                      ...
                      <div class="form-group">
					   <input type="text" class="form-control" id="txtDesig" onchange="validte(this.value.length,'gtxtDesig')"/>
