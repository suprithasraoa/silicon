...
function bindCounter(){
	...
	$("#gtxtClientId").attr('class', 'form-group');
		 ...
		 var url = "sub/get_Counter.php?ctr=Salary27Ctr&field=" +txtClientId;
//alert(url);
    $.getJSON(url, function(result) {
    console.log(result);
    $.each(result, function(i, field) {
    var Salary27Ctr= field.Salary27Ctr;
	$("#txtSerialNo").val(Salary27Ctr);
...
if	(txtClientId==null)	{
    ...
    document.getElementById('sTableRow').innerHTML="<thead><tr><th>#</th><th>User</th><th>Client</th><th>Emp. Name</th><th>Amt</th><th>Tax Deducted Date</th><th>Income Tax</th><th>NatureOfRemit</th><th>Country</th><th>Action</th></tr></thead><tbody>";
    var slno=0;
	var ahref="";

    var url = "sub/bind_list.php?show=" +ddlshowval+"&txtdate="+txtdate+"&txtenddate="+txtenddate+"&cid=0&tbl=SalaryInformation27&CltId="+txtClientId+"&userid="+userid;
    $.getJSON(url, function(result) {
    console.log(result);
    $.each(result, function(i, field) {
    var id= field.Id;
    ...
    $("#sTableRow").append("<tr><td>"+slno+"</td><td style='font-size: 11px;'>"+field.UName+"</td><td style='font-size: 11px;'>"+field.CName+"</td><td style='font-size: 11px;'>"+field.empname+"</td><td style='font-size: 13px;'>"+field.Amount+"</td><td style='font-size: 13px;'>"+TaxDeductedDate+"</td><td style='font-size: 13px;'>"+field.IncTax+"</td><td style='font-size: 13px;'>"+field.NatureOfRemit+"</td><td style='font-size: 13px;'>"+field.CountryofRes+"</td><td>"+ahref);
 ...
 var mnt=moment(txtTaxDeductedDate, 'DD/MM/YYYY').format('MM');
    ...
    var txtAckNo = $("#txtAckNo").val();
	  var txtnatureof = $("#txtnatureof").val();
	    var txtCountry = $("#txtCountry").val();
     var pkvId = $("#pkvId").val();
            var dataString = "txtSerial="+txtSerial+"&txtEmployeeId=" + txtEmployeeId+"&txtAmount="+txtAmount+"&txtTaxDeducted="+txtTaxDeducted+"&txtPaidDate=" + txtPaidDate+"&txtTaxDeductedDate="+txtTaxDeductedDate+"&txtIncTax=" + txtIncTax+"&txtSurchargeAmt=" + txtSurchargeAmt+"&txtEduCessAmt="+txtEduCessAmt+"&txttdsrate=" + txttdsrate+"&txtAckNo=" + txtAckNo+"&txtnatureof=" + txtnatureof+"&txtCountry=" + txtCountry+"&clientId="+txtclientId+"&Tname=SalaryInformation27&pkvId=" + pkvId+"&update=";
 if ($.trim(txtAmount).length > 0 && $.trim(strUsers) > 0  && $.trim(txtPaidDate).length > 0 && $.trim(txtTaxDeductedDate).length > 0 && $.trim(txtIncTax).length > 0 ) {
            ...
            <div class="col-md-12">
              ...
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title">27Q Salary Information</h3>
                </div><!-- /.box-header -->
                <!-- form start -->
                <form role="form">
