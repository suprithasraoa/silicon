...
function bindCounter(){
	...
	$("#gtxtClientId").attr('class', 'form-group');
		 ...
		 var url = "sub/get_Counter.php?ctr=SalaryCtr&field=" +txtClientId;
//alert(url);
    $.getJSON(url, function(result) {
    console.log(result);
    $.each(result, function(i, field) {
    var SalaryCtr= field.SalaryCtr;
	$("#txtSerialNo").val(SalaryCtr);
...
if	(txtClientId==null)	{
    ...
    document.getElementById('sTableRow').innerHTML="<thead><tr><th>#</th><th>User</th><th>Client</th><th>Emp. Name</th><th>Amt</th><th>Tax Deducted Date</th><th>Income Tax</th><th>Challan Sl. No.</th><th>Action</th></tr></thead><tbody>";
    var slno=0;
	var ahref="";

    var url = "sub/bind_list.php?show=" +ddlshowval+"&txtdate="+txtdate+"&txtenddate="+txtenddate+"&cid=0&tbl=SalaryInformation&CltId="+txtClientId+"&userid="+userid;
//alert(url);
    $.getJSON(url, function(result) {
    console.log(result);
    $.each(result, function(i, field) {
    ...
    $("#sTableRow").append("<tr><td>"+slno+"</td><td style='font-size: 11px;'>"+field.UName+"</td><td style='font-size: 11px;'>"+field.CName+"</td><td style='font-size: 11px;'>"+field.empname+"</td><td style='font-size: 13px;'>"+field.Amount+"</td><td style='font-size: 13px;'>"+TaxDeductedDate+"</td><td style='font-size: 13px;'>"+field.IncTax+"</td><td style='font-size: 13px;'>"+field.challanname+"</td><td>"+ahref);
	...
	{
		var txtclientId = $("#txtclientId").val();
		var url = "sub/bind_alltotal.php?Tname=SalaryInformation&clientId="+txtclientId;
//alert(url);
    $.getJSON(url, function(result) {
 ...
 var mnt=moment(txtTaxDeductedDate, 'DD/MM/YYYY').format('MM');
     ...
     var pkvId = $("#pkvId").val();
            var dataString = "txtSerial="+txtSerial+"&txtEmployeeId=" + txtEmployeeId+"&txtAmount="+txtAmount+"&txtTaxDeducted="+txtTaxDeducted+"&txtPaidDate=" + txtPaidDate+"&txtTaxDeductedDate="+txtTaxDeductedDate+"&txtIncTax=" + txtIncTax+"&txtSurchargeAmt=" + txtSurchargeAmt+"&txtEduCessAmt="+txtEduCessAmt+"&txtChallanId=" + txtChallanId+"&txtIncomeTax=" + txtIncomeTax+"&clientId="+txtclientId+"&Tname=SalaryInformation&pkvId=" + pkvId+"&update=";
 if ($.trim(txtAmount).length > 0 && $.trim(strUsers) > 0 && $.trim(strUsers1) > 0 && $.trim(txtPaidDate).length > 0 && $.trim(txtTaxDeductedDate).length > 0 && $.trim(txtIncTax).length > 0 ) {
...
<script>
 ...
 $("#update").css('display','block');
  ...
  $("#save").css('display','none');
   ...
   var txtclientId = $("#txtclientId").val();	
  var url = "sub/bind_list.php?cid="+pkval+"&tbl=SalaryInformation&clientId="+txtclientId;
  //alert(url);
  $.getJSON(url, function(result) {
  ...
  {
  var url = "sub/deletecommon.php?pkvId="+pkval+"&Tname=SalaryInformation";
  //alert(url);
  $.getJSON(url, function(result) {
...
?>
	 ...
	 $query ="SELECT SalaryCtr FROM Parameter where  ClientId='".$_SESSION["loginId"]."'";
...
?>
         ...
         <?php  $query ="SELECT 0  AS Id,'--select--' AS Ename UNION ALL SELECT Id,CONCAT(NAME,' (',SerialNo,') - ',DesignationId) AS Ename FROM Employee where ClientId='".$_SESSION["loginId"]."' ORDER BY Ename ASC";
...
?>
                  ...
                  <!--   <input type="text" class="form-control" id="txtname" placeholder="Name" onchange="validte(this.value.length,'gtxtname')"/> -->
                   ...
                   <div class="col-sm-12">
                   <div class="form-group" >
                       <input type="number" class="form-control" id="txtIncTax" placeholder="Rs." onchange="getamt()" required/>
                    </div>
                  </div>
				   </div>
					 </div>
    ...
    <div class="panel-body">
				 ...
				 <div class="col-sm-6">
                    <div class="form-group" id="gtxtchallanSerialNo">
         ...
         <?php  $query ="SELECT ''  AS Id,'--select--' AS ChallanNo UNION ALL SELECT Id,CONCAT(SerialNo,':Quarter ',QuarterId)AS ChallanNo  FROM Challan  where ClientId='".$_SESSION["loginId"]."'";
 ...
 <div class="col-sm-6">
     ...
     <input type="hidden" class="form-control" id="txtclientId" value=<?php echo $_SESSION["loginId"];  ?> >
          ...
          <div class="col-lg-1">
                ...
                <div class="box-body">
                  <div id="example1_wrapper" class="dataTables_wrapper form-inline" role="grid"><div class="row">  <div class="col-sm-4 class="form-group">
